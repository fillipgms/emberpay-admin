...
const aplicacaoItems: FilterItem[] = [
    { value: "all", label: "Todas" },
    { value: "WEB", label: "WEB" },
    { value: "APP", label: "APP" },
    { value: "API", label: "API" },
...
const TransactionFilters = ({ data }: { data: TransactionsResponse }) => {
    ...
    const handleUserChange = (value: string | null) => {
        const newValue = value === selectedUser ? null : value;
        setSelectedUser(newValue);
        handleUpdateFilters("user", newValue ?? "");
    };

    const userItems = data.users.map((user: TransactionUser) => ({
        ...user,
    ...
    return (
        ...
        >
            ...
            <div className="px-8 py-5 flex items-end gap-6 flex-wrap">
                ...
                <FilterGroup>
                    ...
                    >
                        <ComboboxInput
                            ...
                            className="
                                h-9 px-3 w-48 rounded-lg text-sm
                                bg-[oklch(from_var(--background)_calc(l+0.06)_c_h)]
                                border border-[oklch(from_var(--foreground)_l_c_h_/_0.08)]
                                text-foreground placeholder:text-[oklch(from_var(--foreground)_l_c_h_/_0.35)]
                                hover:border-[oklch(from_var(--primary)_l_c_h_/_0.4)]
                ...
                <FilterGroup>
                    <FilterLabel>Aplicação</FilterLabel>
                    <FilterCombobox
                        items={aplicacaoItems}
                        param="apply"
                        onValueChange={handleUpdateFilters}
                    />
                </FilterGroup>
